...
class SoftbanCommand extends Command {
      ...
      args: [
        {
          ...
          type: "nonModMember",
          prompt: {
            start: "What member would you like to softban?",
            retry: (message, { failure }) => `${failure.value} Try again...`,
          },
        },
      ],
      description: {
        content: `Kicks and prunes a member's messages for the last 24 hours and DMs them with a separate reason.`,
        usage: "<Member>",
  ...
  async exec(message, { member }) {
    ...
    await member.ban({ days: 1, reason: `Softban: ${reason}` }).catch((e) => {
      webhook.send(`**Error:** This user was NOT kicked! Please kick them manually now!`);
    });
    await message.guild.members.unban(member.id, "Softban").catch((e) => {
      webhook.send(`**Error:** This user was NOT unbanned. Please unban them manually now!`);
    });
